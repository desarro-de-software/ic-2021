...
class MemberStatusService {
  ...
  getDebtors(members) {
    ...
    return members.filter((member) => member.balance < 0 && member.lastPayment > threeMonthsAgo);
  }
  ...
  getActive(members) {
    const oneMonth = dayjs(new Date()).subtract(1, 'months');
    return members.filter((member) => member.balance >= 0 && member.lastPayment < oneMonth);
  }
}
